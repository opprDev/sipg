...
jQuery(function($){
      ...
      jQuery(document).on('click', 'input.select-img', function( event ) {
      	...
      	var BusicessClubCustomThemeImage = wp.media.controller.Library.extend({
      		defaults : _.defaults({
        ...
        business_club_file_frame = wp.media.frames.business_club_file_frame = wp.media({
        	button: {
        		text: jQuery( this ).data( 'uploader_button_text' )
        	},
        	state : 'custom-theme-insert-image',
        	states : [
        	new BusicessClubCustomThemeImage()
        	],
        	multiple: false
        });

        // When an image is selected, run a callback.
        business_club_file_frame.on( 'select', function() {

        	var state = business_club_file_frame.state('custom-theme-insert-image');
