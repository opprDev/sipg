...
if ( ! function_exists( 'business_club_register_widgets' ) ) :
	 ...
	 * @since 1.0.0
	 */
	function business_club_register_widgets() {

		// Social widget.
		register_widget( 'Business_Club_Social_Widget' );

		// Featured Page widget.
		register_widget( 'Business_Club_Featured_Page_Widget' );

		// Call To Action widget.
		register_widget( 'Business_Club_Call_To_Action_Widget' );

		// Latest News widget.
		register_widget( 'Business_Club_Latest_News_Widget' );

		// Services widget.
		register_widget( 'Business_Club_Services_Widget' );
	}

endif;

add_action( 'widgets_init', 'business_club_register_widgets' );

if ( ! class_exists( 'Business_Club_Social_Widget' ) ) :
	 ...
	 * @since 1.0.0
	 */
	class Business_Club_Social_Widget extends WP_Widget {
		...
		function __construct() {
			$opts = array(
				'classname'                   => 'business_club_widget_social',
				'description'                 => esc_html__( 'Social Icons Widget', 'business-club' ),
				'customize_selective_refresh' => true,
				);
			parent::__construct( 'business-club-social', esc_html__( 'BC: Social', 'business-club' ), $opts );
		}
		...
		function form( $instance ) {
			...
			<?php if ( true !== has_nav_menu( 'social' ) ) : ?>
				<p><?php echo esc_html__( 'Social menu is not set. Please create menu and assign it to Social Menu.', 'business-club' ); ?></p>
			<?php endif; ?>
			<?php
		}
	}
...
if ( ! class_exists( 'Business_Club_Featured_Page_Widget' ) ) :
	 ...
	 * @since 1.0.0
	 */
	class Business_Club_Featured_Page_Widget extends WP_Widget {
		...
		function __construct() {
			$opts = array(
				'classname'                   => 'business_club_widget_featured_page',
				'description'                 => esc_html__( 'Displays single featured Page', 'business-club' ),
				'customize_selective_refresh' => true,
				);
			parent::__construct( 'business-club-featured-page', esc_html__( 'BC: Featured Page', 'business-club' ), $opts );
		}
					...
					while ( $the_query->have_posts() ) {
						...
						if ( 'short' === $content_type ) {
							if ( absint( $excerpt_length ) > 0 ) {
								...
								echo '<a href="' . esc_url( get_permalink() ) . '" class="read-more">' . esc_html__( 'Read more', 'business-club' ) . '</a>';
							}
						} else {
							the_content();
						}
		...
		function form( $instance ) {
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'featured_page' ) ); ?>"><?php esc_html_e( 'Select Page:', 'business-club' ); ?></label>
				<?php
				wp_dropdown_pages( array(
					'id'               => $this->get_field_id( 'featured_page' ),
					'name'             => $this->get_field_name( 'featured_page' ),
					'selected'         => $instance['featured_page'],
					'show_option_none' => esc_html__( '&mdash; Select &mdash;', 'business-club' ),
					)
				);
				?>
			</p>
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'content_type' ) ); ?>"><?php esc_html_e( 'Show Content:', 'business-club' ); ?></label>
				<select id="<?php echo esc_attr( $this->get_field_id( 'content_type' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'content_type' ) ); ?>">
					<option value="short"<?php selected( $instance['content_type'], 'short' ) ?>><?php esc_html_e( 'Short', 'business-club' ) ?></option>
					<option value="full"<?php selected( $instance['content_type'], 'full' ) ?>><?php esc_html_e( 'Full', 'business-club' ) ?></option>
				</select>
			</p>
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'excerpt_length' ) ); ?>"><?php esc_html_e( 'Excerpt Length:', 'business-club' ); ?></label>
				<input id="<?php echo esc_attr( $this->get_field_id( 'excerpt_length' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'excerpt_length' ) ); ?>" type="number" value="<?php echo esc_attr( $instance['excerpt_length'] ); ?>" min="1" max="200" />&nbsp;<small><?php esc_html_e( 'in words', 'business-club' ); ?></small>
				<br/><span><small><?php esc_html_e( 'Applies when Short is selected in Show Content.', 'business-club') ?></small></span>
			</p>
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'featured_image' ) ); ?>"><?php esc_html_e( 'Featured Image:', 'business-club' ); ?></label>
				<?php
				$dropdown_args = array(
				...
				business_club_render_select_dropdown( $dropdown_args, 'business_club_get_image_sizes_options' );
				?>
			</p>
			<p>
				<label for="<?php echo $this->get_field_id( 'featured_image_alignment' ); ?>"><?php esc_html_e( 'Image Alignment:', 'business-club' ); ?></label>
				<?php
				$dropdown_args = array(
				...
				business_club_render_select_dropdown( $dropdown_args, 'business_club_get_image_alignment_options' );
				?>
			</p>
			<?php
		}
...
if ( ! class_exists( 'Business_Club_Call_To_Action_Widget' ) ) :
	 ...
	 * @since 1.0.0
	 */
	class Business_Club_Call_To_Action_Widget extends WP_Widget {
		...
		function __construct() {
			$opts = array(
				'classname'                   => 'business_club_widget_call_to_action',
				'description'                 => esc_html__( 'Call To Action Widget', 'business-club' ),
				'customize_selective_refresh' => true,
				);
			parent::__construct( 'business-club-call-to-action', esc_html__( 'BC: Call To Action', 'business-club' ), $opts );
		}
		...
		function form( $instance ) {
			...
			$instance = wp_parse_args( (array) $instance, array(
				'title'                 => '',
				'text'                  => '',
				'primary_button_text'   => esc_html__( 'Read more', 'business-club' ),
				'primary_button_url'    => home_url( '/' ),
				'secondary_button_text' => esc_html__( 'Learn more', 'business-club' ),
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'text' ) ); ?>"><?php esc_html_e( 'Text:', 'business-club' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'primary_button_text' ) ); ?>"><?php esc_html_e( 'Primary Button Text:', 'business-club' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'primary_button_url' ) ); ?>"><?php esc_html_e( 'Primary Button URL:', 'business-club' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'secondary_button_text' ) ); ?>"><?php esc_html_e( 'Secondary Button Text:', 'business-club' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'secondary_button_url' ) ); ?>"><?php esc_html_e( 'Secondary Button URL:', 'business-club' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'layout' ) ); ?>"><?php esc_html_e( 'Select Layout:', 'business-club' ); ?></label>
				<?php
				$dropdown_args = array(
				...
				business_club_render_select_dropdown( $dropdown_args, 'business_club_get_numbers_dropdown_options', array( 'min' => 1, 'max' => 2, 'prefix' => esc_html__( 'Layout', 'business-club' ) . ' ' ) );
				?>
			</p>
			<div>
				<label for="<?php echo esc_attr( $this->get_field_id( 'background_image' ) ); ?>"><?php esc_html_e( 'Background Image:', 'business-club' ); ?></label>
				<?php
				...
				<input type="button" class="select-img button button-primary" value="<?php esc_html_e( 'Upload', 'business-club' ); ?>" data-uploader_title="<?php esc_attr_e( 'Select Image', 'business-club' ); ?>" data-uploader_button_text="<?php esc_attr_e( 'Choose Image', 'business-club' ); ?>" />
				<input type="button" value="<?php echo esc_attr_x( 'X', 'remove button', 'business-club' ); ?>" class="button button-secondary btn-image-remove" style="<?php echo esc_attr( $remove_button_style ); ?>" />
				<div class="image-preview-wrap">
					<?php if ( ! empty( $background_image ) ) : ?>
						<img src="<?php echo esc_url( $background_image ); ?>" alt="" />
					<?php endif; ?>
				</div><!-- .image-preview-wrap -->
				<br /><small><?php esc_html_e( 'Background Image applies to Layout 2 only.', 'business-club' ); ?></small>
			</div>
			<?php
		}
	}
...
if ( ! class_exists( 'Business_Club_Services_Widget' ) ) :
	 ...
	 * @since 1.0.0
	 */
	class Business_Club_Services_Widget extends WP_Widget {
		...
		function __construct() {
			...
			$this->block_count = 4;
			$opts = array(
				'classname'                   => 'business_club_widget_services',
				'description'                 => esc_html__( 'Show your services pages with icon and read more link.', 'business-club' ),
				'customize_selective_refresh' => true,
				);
			parent::__construct( 'business-club-services', esc_html__( 'BC: Services', 'business-club' ), $opts );
		}
								...
								<div class="service-block-inner-content">
									...
									<?php if ( absint( $args['excerpt_length'] ) > 0 ) : ?>
										<div class="service-block-item-excerpt">
											<?php
												$excerpt = business_club_get_the_excerpt( absint( $args['excerpt_length'] ) );
												echo wp_kses_post( wpautop( $excerpt ) );
											?>
		...
		function form( $instance ) {
			...
			<?php for ( $i = 1; $i <= $this->block_count ; $i++ ) { ?>
				<h4 class="block-heading"><?php printf( esc_html__( 'Block %d','business-club' ), $i ); ?></h4>
				<p>
					<label for="<?php echo $this->get_field_id( 'block_page_' . $i ); ?>"><?php esc_html_e( 'Page:', 'business-club' ); ?></label>
					<?php
					wp_dropdown_pages( array(
				...
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'block_icon_' . $i ) ); ?>"><?php esc_html_e( 'Icon:', 'business-club' ); ?></label>
					<input id="<?php echo esc_attr( $this->get_field_id( 'block_icon_' . $i ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'block_icon_' . $i ) ); ?>" type="text" value="<?php echo esc_attr( $instance[ 'block_icon_' . $i ] ); ?>" placeholder="<?php esc_attr_e( 'eg: fa-cogs', 'business-club' ); ?>" />
				</p>
				<?php if ( 1 === $i ) : ?>
					<p>
						<a href="<?php echo esc_url( 'http://fontawesome.io/cheatsheet/' ); ?>"><?php esc_html_e( 'View Font Awesome Reference', 'business-club' ); ?></a>
					</p>
				<?php endif; ?>
			<?php } // End for loop. ?>
			<?php
